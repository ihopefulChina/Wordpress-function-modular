...
class Code_Snippets_Edit_Menu extends Code_Snippets_Admin_Menu {
	...
	public function __construct() {
		parent::__construct( 'edit',
			_x( '编辑函数代码', 'menu label', 'code-snippets' ),
			__( '编辑函数代码', 'code-snippets' )
		);
	}
	...
	public function register() {
		...
		$this->add_menu(
			...
			_x( '添加', 'menu label', 'code-snippets' ),
			__( '添加新的函数代码', 'code-snippets' )
		);
	}
	...
	public function render_priority_setting( Code_Snippet $snippet ) {
		...
		<p class="snippet-priority"
		   title="<?php esc_attr_e( 'Snippets with a lower priority number will run before those with a higher number.', 'code-snippets' ); ?>">
			<label for="snippet_priority"><?php esc_html_e( '优先级', 'code-snippets' ); ?></label>
	...
	function render_scope_setting( Code_Snippet $snippet ) {
		...
		$labels = array(
			'global'     => __( '在所有地方运行', 'code-snippets' ),
			'admin'      => __( '仅在管理区域运行', 'code-snippets' ),
			'front-end'  => __( '仅在站点前端运行', 'code-snippets' ),
			'single-use' => __( '只运行一次', 'code-snippets' ),
		);
